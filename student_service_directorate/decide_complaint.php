...
$complaint_query = "
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.escalated_to = 'student_service_directorate'";
...
$stmt_complaint->bind_param("iii", $complaint_id, $escalation_id, $ssd_id);
...
if ($complaint_result->num_rows === 0) {
    ...
    $debug_info = "Complaint ID: $complaint_id, Escalation ID: $escalation_id, SSD ID: $ssd_id\n";
...
// Fetch the user who escalated the complaint to Student Service Directorate (e.g., Cost Sharing)
$escalator = null;
    ...
    if (empty($errors)) {
        ...
        try {
            if ($action === 'resolve') {
                ...
                $notification_desc = "Your complaint #$complaint_id has been resolved by {$ssd['fname']} {$ssd['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $ssd_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you escalated, has been resolved by {$ssd['fname']} {$ssd['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $ssd_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you handled, has been resolved by {$ssd['fname']} {$ssd['lname']}: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $ssd_id, 'resolved', $additional_info)) {
                ...
                $stmt_decision->bind_param("iiiis", $complaint_id, $escalation_id, $ssd_id, $send_back_to_id, $resolution_details);
                ...
                $res_detail = "Sent back to $send_back_to_role by Student Service Directorate with decision: $resolution_details";
                ...
                $recipient_notification_desc = "A decision requires your attention for Complaint #$complaint_id (sent by Student Service Directorate): $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been reviewed by the Student Service Directorate and sent back to the $send_back_to_role for further action.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($other_party_id && $other_party_id != $ssd_id && $other_party_id != $send_back_to_id) {
                    $notify_other_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $other_notification_desc = "Complaint #$complaint_id has been sent back to the $send_back_to_role by the Student Service Directorate: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $ssd_id, 'decision_sent_back', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
                ...
                $stmt_escalation->bind_param("isiii", $complaint_id, $escalated_to_role, $escalated_to_id, $ssd_id, $complaint['original_handler_id']);
                ...
                $res_detail = "Escalated to Administrative Vice President by Student Service Directorate. Reason: $resolution_details";
                $update_escalation_stmt->bind_param("si", $res_detail, $escalation_id);
                $update_escalation_stmt->execute();
                $update_escalation_stmt->close();
                ...
                $escalated_notification_desc = "Complaint #$complaint_id has been escalated to you (Administrative Vice President) by the Student Service Directorate for review. Reason: $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been escalated to the Administrative Vice President by the Student Service Directorate.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $ssd_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $escalator_notification_desc = "Complaint #$complaint_id, which you escalated, has been further escalated to the Administrative Vice President by the Student Service Directorate. Reason: $resolution_details";
                    $notify_escalator_stmt = $db->prepare($notify_escalator_sql);
                    if (!$notify_escalator_stmt) {
                        throw new Exception("An error occurred while notifying the escalator.");
                    }
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $ssd_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $handler_notification_desc = "Complaint #$complaint_id, which you handled, has been escalated by the Student Service Directorate to the Administrative Vice President. Reason: $resolution_details";
                    $notify_handler_stmt = $db->prepare($notify_handler_sql);
                    if (!$notify_handler_stmt) {
                        throw new Exception("An error occurred while notifying the handler.");
                    }
                ...
                // Send stereotyped report to the President
                $additional_info = "Escalated by Student Service Directorate to Administrative Vice President. Reason: $resolution_details";
                if (!sendStereotypedReport($db, $complaint_id, $ssd_id, 'escalated', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
