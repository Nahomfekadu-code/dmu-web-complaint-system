...
$ssd = null; // Initialize $ssd

// Fetch Student Service Directorate details (Needed for the navigation bar)
...
if ($stmt_ssd) {
    ...
    if ($result_ssd->num_rows > 0) {
        ...
        error_log("Student Service Directorate details not found for ID: " . $ssd_id);
    }
    $stmt_ssd->close();
} else {
...
// Fetch resolved complaints where the Student Service Directorate was involved
$stmt = $db->prepare("
    ...
    WHERE e.escalated_to = 'student_service_directorate'
    AND e.escalated_to_id = ?
    AND e.status = 'resolved'
    ORDER BY e.resolved_at DESC
");
if ($stmt) {
    $stmt->bind_param("i", $ssd_id);
    $stmt->execute();
