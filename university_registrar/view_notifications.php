...
if (isset($_GET['mark_read']) && is_numeric($_GET['mark_read'])) {
    ...
    if ($update_stmt) {
        $update_stmt->bind_param("ii", $notification_id, $registrar_id);
        $update_stmt->execute();
...
if (isset($_GET['mark_all_read']) && $_GET['mark_all_read'] === '1') {
    ...
    if ($update_all_stmt) {
        $update_all_stmt->bind_param("i", $registrar_id);
...
$notif_query = "
    ...
    LEFT JOIN escalations e ON c.id = e.complaint_id AND e.escalated_to = 'university_registrar' AND e.escalated_to_id = ?
    WHERE n.user_id = ?
    ORDER BY n.created_at DESC";
$notif_stmt = $db->prepare($notif_query);
if ($notif_stmt) {
    $notif_stmt->bind_param("ii", $registrar_id, $registrar_id);
    $notif_stmt->execute();
...
<head>
    ...
    <title>Notifications | University Registrar - DMU Complaint System</title>
