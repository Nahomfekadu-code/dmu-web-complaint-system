...
// Pending complaints (escalated to University Registrar, status 'pending')
$pending_query = "
    SELECT COUNT(*) as count 
    FROM complaints c
    JOIN escalations e ON c.id = e.complaint_id
    WHERE e.escalated_to = 'university_registrar' 
    AND e.escalated_to_id = ? 
    AND e.status = 'pending'";
$pending_stmt = $db->prepare($pending_query);
if ($pending_stmt) {
    $pending_stmt->bind_param("i", $registrar_id);
...
// In-progress complaints (escalated to University Registrar, status 'in_progress')
$in_progress_query = "
    SELECT COUNT(*) as count 
    FROM complaints c
    JOIN escalations e ON c.id = e.complaint_id
...
if ($in_progress_stmt) {
    $in_progress_stmt->bind_param("i", $registrar_id);
...
// Resolved complaints (handled by University Registrar)
$resolved_query = "
    SELECT COUNT(*) as count 
    FROM complaints c
    JOIN escalations e ON c.id = e.complaint_id
...
if ($resolved_stmt) {
    $resolved_stmt->bind_param("i", $registrar_id);
...
if ($stmt_complaints) {
    $stmt_complaints->bind_param("i", $registrar_id);
...
// Fetch complaints escalated to Academic Vice President by this University Registrar
$escalated_to_academic_vp = [];
$academic_vp_query = "
    SELECT c.id, c.title, c.description, c.category, c.status as complaint_status, 
           e.status as escalation_status, e.created_at as escalated_at
    FROM complaints c
    JOIN escalations e ON c.id = e.complaint_id
    WHERE e.escalated_to = 'academic_vp' 
    AND e.escalated_by_id = ?
    ORDER BY e.created_at DESC";
$stmt_academic_vp = $db->prepare($academic_vp_query);
if ($stmt_academic_vp) {
    $stmt_academic_vp->bind_param("i", $registrar_id);
    $stmt_academic_vp->execute();
    $result_academic_vp = $stmt_academic_vp->get_result();
    while ($row = $result_academic_vp->fetch_assoc()) {
        $escalated_to_academic_vp[] = $row;
    }
    $stmt_academic_vp->close();
} else {
    error_log("Error preparing Academic VP escalated complaints query: " . $db->error);
    $_SESSION['error'] = "An error occurred while fetching complaints escalated to Academic VP.";
}
...
<head>
    ...
    <title>Dashboard | University Registrar - DMU Complaint System</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            <h2>University Registrar Dashboard</h2>
                        ...
                        <tbody>
                            <?php foreach ($complaints as $complaint): ?>
                                <tr>
                                    ...
                                    <td><?php echo date('M j, Y H:i', strtotime($complaint['escalated_at'])); ?></td>
                                    <td>
                                        <?php if ($complaint['escalation_status'] != 'resolved'): ?>
                                            ...
                                            <a href="escalate_complaint.php?complaint_id=<?php echo $complaint['id']; ?>&escalation_id=<?php echo $complaint['escalation_id']; ?>" class="action-btn btn-escalate">
            ...
            <h2>Complaints Escalated to Academic Vice President</h2>
            <div class="table-container">
                <?php if (empty($escalated_to_academic_vp)): ?>
                    <p class="no-complaints">You have not escalated any complaints to the Academic Vice President.</p>
                <?php else: ?>
                    <table>
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            <?php foreach ($escalated_to_academic_vp as $complaint): ?>
                                <tr>
                                    <td><?php echo htmlspecialchars($complaint['id']); ?></td>
                                    <td><?php echo htmlspecialchars($complaint['title']); ?></td>
                                    <td><?php echo htmlspecialchars(ucfirst($complaint['category'] ?? 'Not categorized')); ?></td>
