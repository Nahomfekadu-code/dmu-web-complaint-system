...
if ($stmt_library) {
    ...
    header("Location: library_service_dashboard.php");
    exit;
}
...
$complaint_query = "
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.escalated_to = 'library_service'";
...
$stmt_complaint->bind_param("iii", $complaint_id, $escalation_id, $library_id);
...
if ($complaint_result->num_rows === 0) {
    ...
    $debug_info = "Complaint ID: $complaint_id, Escalation ID: $escalation_id, Library ID: $library_id\n";
...
// Fetch the user who escalated the complaint to Library Service
$escalator = null;
    ...
    if (empty($errors)) {
        ...
        try {
            if ($action === 'resolve') {
                ...
                $notification_desc = "Your complaint #$complaint_id has been resolved by {$library['fname']} {$library['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $library_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you escalated, has been resolved by {$library['fname']} {$library['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $library_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you handled, has been resolved by {$library['fname']} {$library['lname']}: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $library_id, 'resolved', $additional_info)) {
                ...
                $stmt_decision->bind_param("iiiis", $complaint_id, $escalation_id, $library_id, $send_back_to_id, $resolution_details);
                ...
                $res_detail = "Sent back to $send_back_to_role by Library Service with decision: $resolution_details";
                ...
                $recipient_notification_desc = "A decision requires your attention for Complaint #$complaint_id (sent by Library Service): $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been reviewed by the Library Service and sent back to the $send_back_to_role for further action.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($other_party_id && $other_party_id != $library_id && $other_party_id != $send_back_to_id) {
                    $notify_other_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $other_notification_desc = "Complaint #$complaint_id has been sent back to the $send_back_to_role by the Library Service: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $library_id, 'decision_sent_back', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
                ...
                $stmt_escalation->bind_param("isiii", $complaint_id, $escalated_to_role, $escalated_to_id, $library_id, $complaint['original_handler_id']);
                ...
                $res_detail = "Escalated to Student Service Directorate by Library Service. Reason: $resolution_details";
                $update_escalation_stmt->bind_param("si", $res_detail, $escalation_id);
                $update_escalation_stmt->execute();
                $update_escalation_stmt->close();
                ...
                $escalated_notification_desc = "Complaint #$complaint_id has been escalated to you (Student Service Directorate) by the Library Service for review. Reason: $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been escalated to the Student Service Directorate by the Library Service.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $library_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $escalator_notification_desc = "Complaint #$complaint_id, which you escalated, has been further escalated to the Student Service Directorate by the Library Service. Reason: $resolution_details";
                    $notify_escalator_stmt = $db->prepare($notify_escalator_sql);
                    if (!$notify_escalator_stmt) {
                        throw new Exception("An error occurred while notifying the escalator.");
                    }
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $library_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $handler_notification_desc = "Complaint #$complaint_id, which you handled, has been escalated by the Library Service to the Student Service Directorate. Reason: $resolution_details";
                    $notify_handler_stmt = $db->prepare($notify_handler_sql);
                    if (!$notify_handler_stmt) {
                        throw new Exception("An error occurred while notifying the handler.");
                    }
                ...
                // Send stereotyped report to the President
                $additional_info = "Escalated by Library Service to Student Service Directorate. Reason: $resolution_details";
                if (!sendStereotypedReport($db, $complaint_id, $library_id, 'escalated', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
            ...
            header("Location: library_service_decide_complaint.php?complaint_id=$complaint_id&escalation_id=$escalation_id");
            exit;
        }
    } else {
    ...
    <div class="main-content">
        <nav class="horizontal-nav">
            ...
            <div class="horizontal-menu">
                <a href="library_service_dashboard.php" class="<?php echo $current_page == 'library_service_dashboard.php' ? 'active' : ''; ?>">
        ...
        <div class="content-container">
            ...
            <form method="POST" action="library_service_decide_complaint.php?complaint_id=<?php echo $complaint_id; ?>&escalation_id=<?php echo $escalation_id; ?>" novalidate>
                ...
                <div class="form-actions">
                    ...
                    <a href="library_service_dashboard.php" class="btn btn-secondary"><i class="fas fa-times"></i> Cancel</a>
                </div>
            </form>
        </div>
