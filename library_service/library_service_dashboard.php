...
// Fetch complaints assigned to the Library Service user
$complaints = [];
$sql_complaints = "
    ...
    WHERE e.escalated_to_id = ? AND e.escalated_to = 'library_service'
    ORDER BY e.created_at DESC";
$stmt_complaints = $db->prepare($sql_complaints);
if ($stmt_complaints) {
    $stmt_complaints->bind_param("i", $library_id);
    $stmt_complaints->execute();
    $result_complaints = $stmt_complaints->get_result();
    while ($row = $result_complaints->fetch_assoc()) {
        $complaints[] = $row;
    }
    error_log("Complaints fetched for library_id=$library_id: " . print_r($complaints, true));
...
<head>
    ...
    <title>Library Service Dashboard - DMU Complaint System</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            <h2>Library Service Dashboard</h2>
                                        ...
                                        <div class="action-buttons">
                                            <?php if (!empty($complaint['id']) && !empty($complaint['escalation_id'])): ?>
                                                ...
                                                <?php if ($complaint['escalation_status'] === 'pending'): ?>
                                                    <a href="library_service_decide_complaint.php?complaint_id=<?php echo htmlspecialchars($complaint['id']); ?>&escalation_id=<?php echo htmlspecialchars($complaint['escalation_id']); ?>" class="btn btn-success">
                                                        <i class="fas fa-gavel"></i> Decide
                                                    </a>
                                                    <a href="library_service_decide_complaint.php?complaint_id=<?php echo htmlspecialchars($complaint['id']); ?>&escalation_id=<?php echo htmlspecialchars($complaint['escalation_id']); ?>&action=escalate" class="btn btn-info">
