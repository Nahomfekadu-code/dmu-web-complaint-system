...
if ($_SESSION['role'] != 'dormitory_service') {
    error_log("User role is {$_SESSION['role']}, not dormitory_service. Redirecting to unauthorized.");
...
$sql_user = "SELECT fname, lname, email, role, password FROM users WHERE id = ?";
...
if ($stmt_user) {
    ...
    if ($result_user->num_rows > 0) {
        ...
        error_log("Dormitory Service user details not found for ID: $user_id");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($update_password) {
            ...
            if ($stmt_update) {
                $stmt_update->bind_param("ssssi", $fname, $lname, $email, $new_password_hashed, $user_id);
            }
        } else {
            $sql_update = "UPDATE users SET fname = ?, lname = ?, email = ? WHERE id = ?";
            $stmt_update = $db->prepare($sql_update);
            if ($stmt_update) {
                $stmt_update->bind_param("sssi", $fname, $lname, $email, $user_id);
            }
        }
        ...
        if ($stmt_update) {
            if ($stmt_update->execute()) {
                ...
                error_log("Error updating profile for user ID $user_id: " . $stmt_update->error);
    ...
    <nav class="vertical-nav">
        <div class="nav-header">
            ...
            <div class="user-profile-mini">
                <i class="fas fa-user-shield"></i>
                <div class="user-info">
                    <h4>Dormitory Service</h4>
                    <p>Role: Dormitory Service</p>
                </div>
            </div>
            <?php endif; ?>
        </div>
            ...
            <div class="form-container">
                <form method="POST" action="edit_profile.php">
                    <div class="form-group">
                        ...
                        <input type="text" id="fname" name="fname" value="<?php echo htmlspecialchars($user['fname']); ?>" placeholder="Enter your first name" required>
                    </div>
                    <div class="form-group">
                        <label for="lname"><i class="fas fa-user"></i> Last Name</label>
                        <input type="text" id="lname" name="lname" value="<?php echo htmlspecialchars($user['lname']); ?>" placeholder="Enter your last name" required>
                    </div>
                    <div class="form-group">
