...
if ($stmt_dorm) {
    ...
    if ($result_dorm->num_rows > 0) {
        ...
        error_log("Dormitory Service details not found for ID: " . $dorm_id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['notification_id'])) {
    ...
    if ($notification_id) {
        ...
        if ($update_stmt) {
            $update_stmt->bind_param("ii", $notification_id, $dorm_id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['mark_all_read'])) {
    ...
    if ($update_all_stmt) {
        $update_all_stmt->bind_param("i", $dorm_id);
...
");
if ($notif_query) {
    $notif_query->bind_param("i", $dorm_id);
    $notif_query->execute();
    $notifications = $notif_query->get_result();
    error_log("Notifications fetched for dorm_id=$dorm_id: " . $notifications->num_rows . " rows");
    $notif_query->close();
} else {
