...
// Role check: Ensure the user is logged in and is a 'dormitory_service'
if (!isset($_SESSION['user_id']) || !isset($_SESSION['role']) || strtolower($_SESSION['role']) !== 'dormitory_service') {
...
$dorm_id = $_SESSION['user_id'];
$dorm = null;

// Fetch Dormitory Service details
$sql_dorm = "SELECT fname, lname, email, role FROM users WHERE id = ?";
$stmt_dorm = $db->prepare($sql_dorm);
if ($stmt_dorm) {
    $stmt_dorm->bind_param("i", $dorm_id);
    $stmt_dorm->execute();
    $result_dorm = $stmt_dorm->get_result();
    if ($result_dorm->num_rows > 0) {
        $dorm = $result_dorm->fetch_assoc();
    } else {
        $_SESSION['error'] = "Dormitory Service details not found.";
        header("Location: ../logout.php");
        exit;
    }
    $stmt_dorm->close();
} else {
    error_log("Error preparing Dormitory Service query: " . $db->error);
...
$complaint_query = "
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.escalated_to = 'dormitory_service'";
...
$stmt_complaint->bind_param("iii", $complaint_id, $escalation_id, $dorm_id);
...
if ($complaint_result->num_rows === 0) {
    ...
    $debug_info = "Complaint ID: $complaint_id, Escalation ID: $escalation_id, Dorm ID: $dorm_id\n";
...
// Fetch the user who escalated the complaint to Dormitory Service
$escalator = null;
    ...
    if (empty($errors)) {
        ...
        try {
            if ($action === 'resolve') {
                ...
                $notification_desc = "Your complaint #$complaint_id has been resolved by {$dorm['fname']} {$dorm['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $dorm_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you escalated, has been resolved by {$dorm['fname']} {$dorm['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $dorm_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you handled, has been resolved by {$dorm['fname']} {$dorm['lname']}: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $dorm_id, 'resolved', $additional_info)) {
                ...
                $stmt_decision->bind_param("iiiis", $complaint_id, $escalation_id, $dorm_id, $send_back_to_id, $resolution_details);
                ...
                $res_detail = "Sent back to $send_back_to_role by Dormitory Service with decision: $resolution_details";
                ...
                $recipient_notification_desc = "A decision requires your attention for Complaint #$complaint_id (sent by Dormitory Service): $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been reviewed by the Dormitory Service and sent back to the $send_back_to_role for further action.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($other_party_id && $other_party_id != $dorm_id && $other_party_id != $send_back_to_id) {
                    $notify_other_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $other_notification_desc = "Complaint #$complaint_id has been sent back to the $send_back_to_role by the Dormitory Service: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $dorm_id, 'decision_sent_back', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
                ...
                $stmt_escalation->bind_param("isiii", $complaint_id, $escalated_to_role, $escalated_to_id, $dorm_id, $complaint['original_handler_id']);
                ...
                $res_detail = "Escalated to Student Service Directorate by Dormitory Service. Reason: $resolution_details";
                $update_escalation_stmt->bind_param("si", $res_detail, $escalation_id);
                $update_escalation_stmt->execute();
                $update_escalation_stmt->close();
                ...
                $escalated_notification_desc = "Complaint #$complaint_id has been escalated to you (Student Service Directorate) by the Dormitory Service for review. Reason: $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been escalated to the Student Service Directorate by the Dormitory Service.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $dorm_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $escalator_notification_desc = "Complaint #$complaint_id, which you escalated, has been further escalated to the Student Service Directorate by the Dormitory Service. Reason: $resolution_details";
                    $notify_escalator_stmt = $db->prepare($notify_escalator_sql);
                    if (!$notify_escalator_stmt) {
                        throw new Exception("An error occurred while notifying the escalator.");
                    }
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $dorm_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $handler_notification_desc = "Complaint #$complaint_id, which you handled, has been escalated by the Dormitory Service to the Student Service Directorate. Reason: $resolution_details";
                    $notify_handler_stmt = $db->prepare($notify_handler_sql);
                    if (!$notify_handler_stmt) {
                        throw new Exception("An error occurred while notifying the handler.");
                    }
                ...
                // Send stereotyped report to the President
                $additional_info = "Escalated by Dormitory Service to Student Service Directorate. Reason: $resolution_details";
                if (!sendStereotypedReport($db, $complaint_id, $dorm_id, 'escalated', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
...
if ($notif_stmt) {
    $notif_stmt->bind_param("i", $dorm_id);
    $notif_stmt->execute();
...
<body>
    <nav class="vertical-nav">
        <div class="nav-header">
            ...
            <div class="user-profile-mini">
                ...
                <div class="user-info">
                    <h4><?php echo htmlspecialchars($dorm['fname'] . ' ' . $dorm['lname']); ?></h4>
                    <p>Dormitory Service</p>
                </div>
            </div>
        </div>
    ...
    <div class="main-content">
        <nav class="horizontal-nav">
            <div class="logo">
                <span>DMU Complaint System - Dormitory Service</span>
            </div>
