...
// Fetch complaints escalated by this Department Head
$sql_escalated = "
    SELECT c.id as complaint_id, c.title, c.description, c.category, c.status, c.created_at, 
           u.fname, u.lname, e.escalated_to
    FROM complaints c
    JOIN users u ON c.user_id = u.id
    JOIN escalations e ON c.id = e.complaint_id
    WHERE e.escalated_by = ? AND e.action_type = 'escalation'";
$stmt_escalated = $db->prepare($sql_escalated);
$stmt_escalated->bind_param("i", $dept_head_id);
$stmt_escalated->execute();
$escalated_complaints = $stmt_escalated->get_result();
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            <div class="page-header">
                <h2>View Escalated Complaints</h2>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <span><i class="fas fa-arrow-up"></i> Escalated Complaints</span>
                </div>
                <div class="card-body">
                    <?php if ($escalated_complaints->num_rows > 0): ?>
                        <div class="table-responsive">
                            <table id="escalatedTable" class="responsive-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th data-sort="created_at">Created On</th>
                                        <th data-sort="complainant">Complainant</th>
                                        <th data-sort="escalated_to">Escalated To</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php while ($complaint = $escalated_complaints->fetch_assoc()): ?>
                                        <tr>
                                            <td data-label="Complaint ID"><?php echo htmlspecialchars($complaint['complaint_id']); ?></td>
                                            <td data-label="Title"><?php echo htmlspecialchars($complaint['title']); ?></td>
                                            <td data-label="Description" class="description-cell"><?php echo nl2br(htmlspecialchars($complaint['description'])); ?></td>
                                            <td data-label="Category"><?php echo htmlspecialchars(ucfirst($complaint['category'])); ?></td>
                                            <td data-label="Status"><?php echo htmlspecialchars(ucfirst($complaint['status'])); ?></td>
                                            <td data-label="Created On"><?php echo date("M j, Y, g:i a", strtotime($complaint['created_at'])); ?></td>
                                            <td data-label="Complainant"><?php echo htmlspecialchars($complaint['fname'] . ' ' . $complaint['lname']); ?></td>
                                            <td data-label="Escalated To"><?php echo htmlspecialchars(ucfirst(str_replace('_', ' ', $complaint['escalated_to']))); ?></td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
                            </table>
                    ...
                    <?php else: ?>
                        <p class="text-muted" style="text-align: center; padding: 20px 0;">No complaints escalated by you.</p>
                    <?php endif; ?>
                    <?php $escalated_complaints->free(); ?>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const escalatedTable = document.getElementById('escalatedTable');
            if (escalatedTable) {
                escalatedTable.querySelectorAll("th[data-sort]").forEach((headerCell) => {
                    ...
                    headerCell.addEventListener("click", () => {
                        ...
                        sortTableByColumn(escalatedTable, index, !currentIsAscending);
                    });
                });
            }
        });
