...
// Fetch resolved complaints assigned to this Department Head
$sql = "
    SELECT c.id as complaint_id, c.title, c.user_id as complainant_id, u.fname, u.lname 
    FROM complaints c 
    JOIN users u ON c.user_id = u.id 
    JOIN escalations e ON c.id = e.complaint_id
    WHERE e.escalated_to = 'department_head' 
    AND e.status = 'resolved'
    AND e.department_id = (SELECT id FROM departments WHERE head_id = ? LIMIT 1)
    AND NOT EXISTS (
        SELECT 1 FROM decisions d 
        WHERE d.complaint_id = c.id 
        AND d.sender_id = ? 
        AND d.receiver_id = c.user_id
    )";
$stmt = $db->prepare($sql);
$stmt->bind_param("ii", $dept_head_id, $dept_head_id);
$stmt->execute();
$resolved_complaints = $stmt->get_result();
$stmt->close();

// Fetch decisions sent to this Department Head (as the receiver)
$sql = "
    SELECT d.id, d.decision_text, d.created_at, c.title, u.fname, u.lname, u.role as sender_role 
    FROM decisions d 
    JOIN complaints c ON d.complaint_id = c.id 
    JOIN users u ON d.sender_id = u.id 
    WHERE d.receiver_id = ? AND d.status = 'pending'";
$stmt = $db->prepare($sql);
$stmt->bind_param("i", $dept_head_id);
$stmt->execute();
$received_decisions = $stmt->get_result();
$stmt->close();

// Check if a complaint_id is passed via URL
$preselected_complaint_id = isset($_GET['complaint_id']) ? filter_input(INPUT_GET, 'complaint_id', FILTER_VALIDATE_INT) : null;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$complaint_id || empty($decision_text)) {
        $_SESSION['error'] = "Please select a complaint and provide a decision text.";
    } else {
        // Fetch complaint details to verify authorization and get complainant_id
        $sql = "
            SELECT c.user_id as complainant_id 
            FROM complaints c 
            JOIN escalations e ON c.id = e.complaint_id
            WHERE c.id = ? 
        ...
        if ($result->num_rows == 0) {
            $_SESSION['error'] = "Invalid complaint ID or you are not authorized to handle this complaint.";
            $stmt->close();
        } else {
            ...
            // Check if a decision has already been sent to the complainant
            $sql_check = "
                SELECT id FROM decisions 
                WHERE complaint_id = ? AND sender_id = ? AND receiver_id = ?";
            $stmt_check = $db->prepare($sql_check);
            $stmt_check->bind_param("iii", $complaint_id, $dept_head_id, $complainant_id);
            $stmt_check->execute();
            $decision_exists = $stmt_check->get_result()->num_rows > 0;
            $stmt_check->close();

            if ($decision_exists) {
                $_SESSION['error'] = "A decision has already been sent to the complainant for this complaint.";
            } else {
                // Begin transaction
                ...
                try {
                    ...
                    $sql = "
                        ...
                        VALUES (NULL, ?, ?, ?, ?, ?, NOW())";
                    $stmt_decision = $db->prepare($sql);
                    $stmt_decision->bind_param("iiiss", $complaint_id, $dept_head_id, $complainant_id, $decision_text, $decision_status);
                    $stmt_decision->execute();
                    $stmt_decision->close();

                    // Insert a notification for the complainant
                    $notification_message = "Your complaint #$complaint_id has been resolved. A decision has been sent to you by the Department Head.";
                    $sql = "
                        INSERT INTO notifications (user_id, complaint_id, description, created_at) 
                        VALUES (?, ?, ?, NOW())";
                    $stmt_notification = $db->prepare($sql);
                    $stmt_notification->bind_param("iis", $complainant_id, $complaint_id, $notification_message);
                    $stmt_notification->execute();
                    $stmt_notification->close();

                    // Commit the transaction
                    $db->commit();
                    $_SESSION['success'] = "Decision sent to complainant successfully!";
                } catch (Exception $e) {
                    $db->rollback();
                    $_SESSION['error'] = "Failed to send decision: " . $e->getMessage();
                    error_log("Error sending decision: " . $e->getMessage());
                }
            }
        }
    }
    
    header("Location: send_decision.php" . ($preselected_complaint_id ? "?complaint_id=$preselected_complaint_id" : ""));
    exit;
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .btn-warning:hover { background-color: #e0a800; transform: translateY(-1px); box-shadow: var(--shadow-hover); }
...
<body>
    ...
    <nav class="vertical-nav">
        ...
        <div class="nav-menu">
            <h3>Dashboard</h3>
            <a href="dashboard.php" class="nav-link <?php echo $current_page == 'department_head_dashboard.php' ? 'active' : ''; ?>">
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            ...
            <!-- Decisions Received from Higher Authorities -->
            <div class="card">
                <div class="card-header">
                    <span><i class="fas fa-gavel"></i> Decisions Received from Higher Authorities</span>
                </div>
                <div class="card-body">
                    <?php if ($received_decisions->num_rows > 0): ?>
                        <div class="table-responsive">
                            <table id="decisionsTable" class="responsive-table">
                                <thead>
                                    <tr>
                                        <th data-sort="title">Complaint Title</th>
                                        <th>Decision Text</th>
                                        <th data-sort="sender">Sent By</th>
                                        <th data-sort="created_at">Sent On</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php while ($decision = $received_decisions->fetch_assoc()): ?>
                                        <tr>
                                            <td data-label="Complaint Title"><?php echo htmlspecialchars($decision['title']); ?></td>
                                            <td data-label="Decision Text" class="description-cell"><?php echo nl2br(htmlspecialchars($decision['decision_text'])); ?></td>
                                            <td data-label="Sent By"><?php echo htmlspecialchars($decision['fname'] . ' ' . $decision['lname'] . ' (' . ucfirst($decision['sender_role']) . ')'); ?></td>
                                            <td data-label="Sent On"><?php echo date("M j, Y, g:i a", strtotime($decision['created_at'])); ?></td>
                                        </tr>
                                    <?php endwhile; ?>
                                </tbody>
                            </table>
                    ...
                    <?php else: ?>
                        <p class="text-muted" style="text-align: center; padding: 20px 0;">No decisions received from higher authorities.</p>
                    <?php endif; ?>
                    <?php $received_decisions->free(); ?>
                </div>
            </div>

            <!-- Send Decision to Complainant -->
            <div class="card">
                <div class="card-header">
                    <span><i class="fas fa-paper-plane"></i> Send Decision to Complainant</span>
                </div>
                <div class="card-body">
                    <?php if ($resolved_complaints->num_rows > 0): ?>
                        <form action="send_decision.php" method="POST">
                            <div class="form-group">
                                <label for="complaint_id">Select Complaint:</label>
                                <select name="complaint_id" id="complaint_id" required>
                                    <option value="">-- Select a Complaint --</option>
                                    <?php 
                                    $resolved_complaints->data_seek(0); // Reset pointer to loop again
                                    while ($complaint = $resolved_complaints->fetch_assoc()): 
                                    ?>
                                        <option value="<?php echo $complaint['complaint_id']; ?>" <?php echo $preselected_complaint_id == $complaint['complaint_id'] ? 'selected' : ''; ?>>
                                            Complaint #<?php echo $complaint['complaint_id']; ?> - <?php echo htmlspecialchars($complaint['title']); ?> (by <?php echo htmlspecialchars($complaint['fname'] . ' ' . $complaint['lname']); ?>)
                                        </option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="decision_text">Decision Text:</label>
                                <textarea name="decision_text" id="decision_text" required placeholder="Enter the decision details..."></textarea>
                            </div>
                    ...
                    <?php else: ?>
                        <p class="text-muted" style="text-align: center; padding: 20px 0;">No resolved complaints available to send decisions for.</p>
                    <?php endif; ?>
                    <?php $resolved_complaints->free(); ?>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (decisionsTable) {
                decisionsTable.querySelectorAll("th[data-sort]").forEach((headerCell) => {
                    ...
                    headerCell.addEventListener("click", () => {
                        ...
                        sortTableByColumn(decisionsTable, index, !currentIsAscending);
                    });
                });
            }
        });
