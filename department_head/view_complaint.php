...
// Check if the complaint is accessible to the department head
$access_check_query = "
    SELECT c.id
    FROM complaints c
    ...
    WHERE c.id = ?
    AND (
        (e.escalated_to = 'department_head' AND e.escalated_to_id = ?)
        OR (d.sender_id = ?)
    )";
$stmt_access = $db->prepare($access_check_query);
$stmt_access->bind_param("iii", $complaint_id, $dept_head_id, $dept_head_id);
$stmt_access->execute();
