...
// Fetch complaint details (only complaints assigned to this Department Head)
$stmt = $db->prepare("
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.action_type = 'assignment' AND e.status = 'pending'
");
if (!$stmt) {
...
$stmt->bind_param("iii", $complaint_id, $escalation_id, $dept_head_id);
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($decision_text)) {
        $_SESSION['error'] = "Please provide decision or resolution details.";
        header("Location: decide_complaint.php?complaint_id=$complaint_id&escalation_id=$escalation_id");
        exit;
    }
    ...
    if (!$action || !in_array($action, ['resolve', 'send_back'])) {
    ...
    try {
        if ($action === 'resolve') {
            ...
            $decision_stmt->bind_param("iiiis", $escalation_id, $complaint_id, $dept_head_id, $handler_id, $decision_text);
            ...
            $notification_desc = "A final decision has been made on Complaint #$complaint_id by {$dept_head['fname']} {$dept_head['lname']}.";
            ...
            $notification_desc = "Your complaint #$complaint_id has been resolved by {$dept_head['fname']} {$dept_head['lname']}: $decision_text";
            ...
            $additional_info = "Resolved by Department Head: $decision_text";
            if (!sendStereotypedReport($db, $complaint_id, $dept_head_id, 'resolved', $additional_info)) {
            ...
            // Action: Send back to Handler with a decision
            // Insert decision
            $decision_sql = "INSERT INTO decisions (escalation_id, complaint_id, sender_id, receiver_id, decision_text, status, created_at) VALUES (?, ?, ?, ?, ?, 'pending', NOW())";
            $decision_stmt = $db->prepare($decision_sql);
            if (!$decision_stmt) {
                throw new Exception("An error occurred while recording the decision.");
            }
            ...
            // Update escalation status to pending and reassign to handler
            $update_escalation_sql = "
                UPDATE escalations 
                SET status = 'pending', 
                    resolution_details = ?, 
                    escalated_to = 'handler', 
                    escalated_to_id = ?, 
                    resolved_at = NULL 
                WHERE id = ?";
            $update_escalation_stmt = $db->prepare($update_escalation_sql);
            ...
            $update_escalation_stmt->bind_param("sii", $decision_text, $handler_id, $escalation_id);
            $update_escalation_stmt->execute();
            $update_escalation_stmt->close();

            // Update complaint status to in_progress
            $update_complaint_sql = "UPDATE complaints SET status = 'in_progress', resolution_details = NULL, resolution_date = NULL WHERE id = ?";
            $update_complaint_stmt = $db->prepare($update_complaint_sql);
            if (!$update_complaint_stmt) {
                throw new Exception("An error occurred while updating the complaint status.");
            }
            ...
            // Notify the Handler
            $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
            $notification_desc = "A decision has been sent back to you for Complaint #$complaint_id by {$dept_head['fname']} {$dept_head['lname']}: $decision_text";
            $notify_handler_stmt = $db->prepare($notify_handler_sql);
            if (!$notify_handler_stmt) {
                throw new Exception("An error occurred while notifying the Handler.");
            }
            ...
            $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) 
                               SELECT user_id, ?, ?, 0, NOW() 
                               FROM complaints WHERE id = ?";
            $notification_desc = "Your complaint #$complaint_id has been updated with a decision by {$dept_head['fname']} {$dept_head['lname']}: $decision_text";
            $notify_user_stmt = $db->prepare($notify_user_sql);
            if (!$notify_user_stmt) {
                throw new Exception("An error occurred while notifying the complainant.");
            }
            ...
            // Send stereotyped report to the President
            $additional_info = "Decision sent back to Handler: $decision_text";
            if (!sendStereotypedReport($db, $complaint_id, $dept_head_id, 'decision_received', $additional_info)) {
                throw new Exception("Failed to send the report to the President.");
            }
        ...
        <div class="content-container">
            ...
            <form method="POST" onsubmit="return confirm('Are you sure you want to submit this decision? This action cannot be undone.');">
                ...
                <div class="form-group">
                    ...
                    <textarea name="decision_text" id="decision_text" rows="5" required placeholder="Enter your decision or resolution details here..."></textarea>
                </div>
                <div class="form-group">
                    ...
                    <select name="action" id="action" required>
                        ...
                        <option value="send_back">Send Decision Back to Handler</option>
                    </select>
                </div>
