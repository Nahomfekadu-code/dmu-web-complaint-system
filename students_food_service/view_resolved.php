...
// Fetch resolved complaints where the Students Food Service was involved
$stmt = $db->prepare("
    ...
    WHERE e.escalated_to = 'students_food_service'
    AND e.escalated_to_id = ?
    AND e.status = 'resolved'
    ORDER BY e.resolved_at DESC
");
if ($stmt) {
    $stmt->bind_param("i", $food_service_id);
    $stmt->execute();
    $resolved_complaints = $stmt->get_result();
    error_log("Resolved complaints fetched for food_service_id=$food_service_id: " . $resolved_complaints->num_rows . " rows");
    $stmt->close();
} else {
