...
// Function to generate text file for decisions (Resolve, Send Back, Escalate)
function generateDecisionTextFile($complaint_id, $escalation_id, $action, $resolution_details, $recipient_role, $db) {
    $decisions_dir = 'decisions/';
    if (!is_dir($decisions_dir)) {
        mkdir($decisions_dir, 0777, true);
    }
    
    $filename = $decisions_dir . "decision_complaint_{$complaint_id}_escalation_{$escalation_id}_" . time() . ".txt";
    
    // Content for the text file
    $content = "DMU Complaint System Decision Report\n";
    $content .= "================================\n";
    $content .= "Complaint ID: $complaint_id\n";
    $content .= "Escalation ID: $escalation_id\n";
    $content .= "Action: " . ucfirst($action) . "\n";
    $content .= "Resolution Details: $resolution_details\n";
    $content .= "Processed By: Students Food Service\n";
    $content .= "Recipient Role: " . ($recipient_role ? ucfirst(str_replace('_', ' ', $recipient_role)) : 'N/A') . "\n";
    $content .= "Date: " . date('M j, Y H:i') . "\n";
    
    // Save the text file
    if (file_put_contents($filename, $content) === false) {
        error_log("Failed to write decision text file: $filename");
        return false;
    }
...
if (!function_exists('sendStereotypedReport')) {
...
$sql_food_service = "SELECT fname, lname, email, role FROM users WHERE id = ?";
...
$complaint_query = "
    SELECT c.id, c.user_id, c.title, c.description, c.category, c.status, c.created_at, 
           e.escalated_by_id, e.original_handler_id, e.action_type, e.status as escalation_status
    FROM complaints c
    JOIN escalations e ON c.id = e.complaint_id
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.escalated_to = 'students_food_service'";
...
$stmt_complaint->bind_param("iii", $complaint_id, $escalation_id, $food_service_id);
$stmt_complaint->execute();
$complaint_result = $stmt_complaint->get_result();
if ($complaint_result->num_rows === 0) {
    $debug_query = "
    ...
    $debug_info = "Complaint ID: $complaint_id, Escalation ID: $escalation_id, Food Service ID: $food_service_id\n";
...
// Fetch the user who escalated the complaint to Students Food Service
$escalator = null;
...
// Fetch original handler details (for "Send Back" option)
$handler = null;
...
if (!$can_send_back_to_escalator && !$can_send_back_to_handler && empty($escalation_options)) {
    error_log("No valid actions available for complaint #$complaint_id. Escalator: " . ($can_send_back_to_escalator ? 'Found' : 'Not Found') . ", Original Handler: " . ($can_send_back_to_handler ? 'Found' : 'Not Found'));
    $_SESSION['error'] = "Error: No valid actions available for this complaint.";
    header("Location: dashboard.php");
    exit;
}

// Fetch users for escalation (only Student Service Directorate)
$roles_to_escalate = ['student_service_directorate'];
$escalation_options = [];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            if ($action === 'resolve') {
                // Generate text file for Resolve action
                $txt_filename = generateDecisionTextFile($complaint_id, $escalation_id, 'resolve', $resolution_details, null, $db);
                if (!$txt_filename || !file_exists($txt_filename)) {
                    throw new Exception("Failed to generate the Resolve decision text file.");
                }
                ...
                $notification_desc = "Your complaint #$complaint_id has been resolved by {$food_service['fname']} {$food_service['lname']}: $resolution_details. Decision File: $txt_filename";
                ...
                // Notify the escalator (e.g., Handler)
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $food_service_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you escalated, has been resolved by {$food_service['fname']} {$food_service['lname']}: $resolution_details. Decision File: $txt_filename";
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $food_service_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you handled, has been resolved by {$food_service['fname']} {$food_service['lname']}: $resolution_details. Decision File: $txt_filename";
                ...
                $additional_info = "Resolved: $resolution_details. Decision File: $txt_filename";
                if (!sendStereotypedReport($db, $complaint_id, $food_service_id, 'resolved', $additional_info)) {
                ...
                // Generate text file for Send Back action
                $txt_filename = generateDecisionTextFile($complaint_id, $escalation_id, 'send_back', $resolution_details, $send_back_to_role, $db);
                if (!$txt_filename || !file_exists($txt_filename)) {
                    throw new Exception("Failed to generate the Send Back decision text file.");
                }

                // Insert decision record with file_path
                $decision_sql = "INSERT INTO decisions (complaint_id, escalation_id, sender_id, receiver_id, decision_text, status, file_path, created_at) VALUES (?, ?, ?, ?, ?, 'pending', ?, NOW())";
                $stmt_decision = $db->prepare($decision_sql);
                if (!$stmt_decision) {
                    throw new Exception("An error occurred while recording the decision.");
                }
                $stmt_decision->bind_param("iiiiss", $complaint_id, $escalation_id, $food_service_id, $send_back_to_id, $resolution_details, $txt_filename);
                ...
                $res_detail = "Sent back to $send_back_to_role by Students Food Service with decision: $resolution_details";
                ...
                $recipient_notification_desc = "A decision requires your attention for Complaint #$complaint_id (sent by Students Food Service): $resolution_details. Decision File: $txt_filename";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been reviewed by the Students Food Service and sent back to the $send_back_to_role for further action.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($other_party_id && $other_party_id != $food_service_id && $other_party_id != $send_back_to_id) {
                    $notify_other_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $other_notification_desc = "Complaint #$complaint_id has been sent back to the $send_back_to_role by the Students Food Service: $resolution_details. Decision File: $txt_filename";
                ...
                // Send stereotyped report to the President
                $additional_info = "Sent back to $send_back_to_role: $resolution_details. Decision File: $txt_filename";
                if (!sendStereotypedReport($db, $complaint_id, $food_service_id, 'decision_sent_back', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
                ...
                // Generate text file for Escalate action
                $txt_filename = generateDecisionTextFile($complaint_id, $escalation_id, 'escalate', $resolution_details, 'student_service_directorate', $db);
                if (!$txt_filename || !file_exists($txt_filename)) {
                    throw new Exception("Failed to generate the Escalate decision text file.");
                }
                ...
                $stmt_escalation->bind_param("isiii", $complaint_id, $escalated_to_role, $escalated_to_id, $food_service_id, $complaint['original_handler_id']);
                ...
                $res_detail = "Escalated to Student Service Directorate by Students Food Service. Reason: $resolution_details";
                $update_escalation_stmt->bind_param("si", $res_detail, $escalation_id);
                $update_escalation_stmt->execute();
                $update_escalation_stmt->close();
                ...
                // Notify the escalated-to user (Student Service Directorate)
                $notify_escalated_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $escalated_notification_desc = "Complaint #$complaint_id has been escalated to you (Student Service Directorate) by the Students Food Service for review. Reason: $resolution_details. Decision File: $txt_filename";
                $notify_escalated_stmt = $db->prepare($notify_escalated_sql);
                if (!$notify_escalated_stmt) {
                    throw new Exception("An error occurred while notifying the Student Service Directorate.");
                }
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been escalated to the Student Service Directorate by the Students Food Service.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $food_service_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $escalator_notification_desc = "Complaint #$complaint_id, which you escalated, has been further escalated to the Student Service Directorate by the Students Food Service. Reason: $resolution_details. Decision File: $txt_filename";
                    $notify_escalator_stmt = $db->prepare($notify_escalator_sql);
                    if (!$notify_escalator_stmt) {
                        throw new Exception("An error occurred while notifying the escalator.");
                    }
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $food_service_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $handler_notification_desc = "Complaint #$complaint_id, which you handled, has been escalated by the Students Food Service to the Student Service Directorate. Reason: $resolution_details. Decision File: $txt_filename";
                    $notify_handler_stmt = $db->prepare($notify_handler_sql);
                    if (!$notify_handler_stmt) {
                        throw new Exception("An error occurred while notifying the handler.");
                    }
                ...
                // Send stereotyped report to the President
                $additional_info = "Escalated by Students Food Service to Student Service Directorate. Reason: $resolution_details. Decision File: $txt_filename";
                if (!sendStereotypedReport($db, $complaint_id, $food_service_id, 'escalated', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }

                $_SESSION['success'] = "Complaint #$complaint_id has been escalated to the Student Service Directorate successfully.";
            }
            ...
            <form method="POST" action="decide_complaint.php?complaint_id=<?php echo $complaint_id; ?>&escalation_id=<?php echo $escalation_id; ?>" novalidate>
                ...
                <div class="form-group">
                    ...
                    <select name="action" id="action" required onchange="toggleAdditionalOptions()">
                        ...
                        <?php endif; ?>
                        <option value="escalate" <?php echo $display_action === 'escalate' ? 'selected' : ''; ?>>Escalate to Student Service Directorate</option>
                    </select>
                </div>
                ...
                <div id="escalation-options">
                    <div class="form-group">
                        <label for="escalation_target_select">Select Student Service Directorate *</label>
                        <select name="escalation_target_select" id="escalation_target_select" onchange="updateEscalationFields(this)">
                            <option value="" disabled selected>-- Select Student Service Directorate --</option>
                            <?php if (!empty($escalation_options['student_service_directorate'])): ?>
                                <?php foreach ($escalation_options['student_service_directorate'] as $user_option): ?>
                                    <?php
                                        $option_value = 'student_service_directorate|' . $user_option['id'];
                                        $is_selected = ($display_escalated_role === 'student_service_directorate' && (int)$display_escalated_id === $user_option['id']);
                                    ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <option value="" disabled>No Student Service Directorate users found.</option>
                            <?php endif; ?>
                        </select>
            ...
            if (form) {
                form.addEventListener('submit', function(event) {
                    ...
                    if (action === 'resolve') {
                        ...
                        confirmMessage = 'Are you sure you want to escalate this complaint to the Student Service Directorate?';
                    }
