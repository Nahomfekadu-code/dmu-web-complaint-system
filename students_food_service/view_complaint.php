...
if (strtolower($_SESSION['role']) !== 'students_food_service') {
    error_log("User role is {$_SESSION['role']}, not students_food_service. Redirecting to unauthorized.");
...
// Fetch Students Food Service user details
...
if ($stmt_user) {
    ...
    if ($result_user->num_rows > 0) {
        ...
        error_log("Students Food Service user details not found for ID: $user_id");
...
// Check if the complaint is accessible to the students food service user
$access_check_query = "
    SELECT c.id
    FROM complaints c
...
$stmt_access->bind_param("iiii", $complaint_id, $user_id, $user_id, $user_id);
$stmt_access->execute();
    ...
    <nav class="vertical-nav">
        <div class="nav-header">
            ...
            <div class="user-profile-mini">
                <i class="fas fa-user-shield"></i>
                <div class="user-info">
                    <h4>Students Food Service</h4>
                    <p>Role: Students Food Service</p>
                </div>
            </div>
            <?php endif; ?>
        </div>
