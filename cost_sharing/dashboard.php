...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['decide_complaint'])) {
    ...
    if (!$complaint_id || $complaint_id <= 0) {
        ...
        try {
            ...
            $stmt->bind_param("iiiss", $complaint_id, $cost_sharing_id, $receiver_id, $decision_text, $status);
            if (!$stmt->execute()) {
            ...
            // Update escalation
            $update_escalation_query = "UPDATE escalations SET status = ?, escalated_to = ?, escalated_to_id = ? WHERE complaint_id = ? AND escalated_to = 'cost_sharing' AND escalated_to_id = ?";
            ...
            $stmt_update->bind_param("ssiii", $new_status, $escalated_to_param, $escalated_to_id_param, $complaint_id, $cost_sharing_id);
            if (!$stmt_update->execute()) {
            ...
            if ($receiver_id) {
                ...
                $notification_desc = ($action === 'escalate') ? "Complaint #$complaint_id escalated to you by Cost Sharing." : "Complaint #$complaint_id sent back to you by Cost Sharing with more info requested.";
...
$sql_count_assigned = "SELECT COUNT(*) as total FROM complaints c JOIN escalations e ON c.id = e.complaint_id WHERE e.escalated_to = 'cost_sharing' AND e.escalated_to_id = ? AND e.status = 'pending'";
$stmt_count_assigned = $db->prepare($sql_count_assigned);
$total_assigned = 0;
if ($stmt_count_assigned) {
    $stmt_count_assigned->bind_param("i", $cost_sharing_id);
...
$sql_assigned = "SELECT c.*, u_submitter.fname as submitter_fname, u_submitter.lname as submitter_lname, e.id as escalation_id, e.status as escalation_status, e.action_type, e.escalated_to, e.escalated_to_id,
                (SELECT d.decision_text FROM decisions d WHERE d.complaint_id = c.id AND d.receiver_id = ? ORDER BY d.created_at DESC LIMIT 1) as latest_decision
                 ...
                 WHERE e.escalated_to = 'cost_sharing' AND e.escalated_to_id = ? AND e.status = 'pending' 
                 ORDER BY c.created_at DESC LIMIT ? OFFSET ?";
$stmt_assigned = $db->prepare($sql_assigned);
$assigned_complaints = [];
if ($stmt_assigned) {
    $stmt_assigned->bind_param("iiii", $cost_sharing_id, $cost_sharing_id, $items_per_page, $offset_assigned);
...
// Stats tailored for Cost Sharing
$stats = [
...
if ($stmt_stats_resolved) {
    $stmt_stats_resolved->bind_param("i", $cost_sharing_id);
...
if ($stmt_stats_sent_back) {
    $stmt_stats_sent_back->bind_param("i", $cost_sharing_id);
...
if ($stmt_stats_escalated) {
    $stmt_stats_escalated->bind_param("i", $cost_sharing_id);
...
<head>
    ...
    <title>Cost Sharing Dashboard | DMU Complaint System</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            <h2>Welcome, <?php echo htmlspecialchars($cost_sharing_user['fname'] . ' ' . $cost_sharing_user['lname']); ?>!</h2>
            ...
            <div class="user-profile">
                ...
                <div class="profile-card">
                    ...
                    <div class="profile-details">
                        <p><strong>Name:</strong> <?php echo htmlspecialchars($cost_sharing_user['fname'] . ' ' . $cost_sharing_user['lname']); ?></p>
                        <p><strong>Email:</strong> <?php echo htmlspecialchars($cost_sharing_user['email']); ?></p>
                        <p><strong>Role:</strong> Cost Sharing</p>
                    </div>
                </div>
            </div>
            <?php endif; ?>
                                ...
                                <?php foreach ($assigned_complaints as $complaint): ?>
                                    <tr>
                                        ...
                                        <td data-label="Actions">
                                            ...
                                            <?php if (isset($complaint['escalation_status'], $complaint['escalated_to'], $complaint['escalated_to_id']) && $complaint['escalation_status'] === 'pending' && $complaint['escalated_to'] === 'cost_sharing' && $complaint['escalated_to_id'] == $cost_sharing_id): ?>
