...
// Fetch open complaints assigned to the cost_sharing user
...
if ($stmt_complaints) {
    $stmt_complaints->bind_param("i", $cost_sharing_id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['escalate']) && $campus_registrar_id) {
    ...
    if ($stmt_update->execute()) {
        ...
        $sql_escalate_update = "UPDATE escalations 
                                ...
                                WHERE complaint_id = ? AND escalated_to = 'cost_sharing' AND escalated_to_id = ?";
        $stmt_escalate_update = $db->prepare($sql_escalate_update);
        $stmt_escalate_update->bind_param("iii", $campus_registrar_id, $complaint_id, $cost_sharing_id);
        $stmt_escalate_update->execute();
        $stmt_escalate_update->close();

        // Create a notification for the campus registrar
        $message = "A complaint (ID: $complaint_id) has been escalated to you by Cost Sharing.";
