...
$sql_user = "SELECT fname, lname, phone, sex, email FROM users WHERE id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        // Update user details
        $update_sql = "UPDATE users SET fname = ?, lname = ?, phone = ?, sex = ?, email = ? WHERE id = ?";
        $stmt_update = $db->prepare($update_sql);
        if ($stmt_update) {
            $stmt_update->bind_param("sssssi", $fname, $lname, $phone, $sex, $email, $user_id);
...
<head>
    ...
    <title>Edit Profile | DMU Registrar Complaint System</title>
