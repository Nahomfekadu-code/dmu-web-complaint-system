...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'cost_sharing') {
...
// Fetch complaint details (only complaints assigned to this Cost Sharing user)
$stmt = $db->prepare("
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to = 'cost_sharing' AND e.escalated_to_id = ? AND e.status = 'pending'
");
if (!$stmt) {
...
$stmt->bind_param("iii", $complaint_id, $escalation_id, $cost_sharing_id);
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        if ($action === 'resolve') {
            ...
            $decision_stmt->bind_param("iiiis", $escalation_id, $complaint_id, $cost_sharing_id, $handler_id, $decision_text);
            ...
            if ($handler_id) {
                ...
                $notification_desc = "A final decision has been made on Complaint #$complaint_id by Cost Sharing.";
            ...
            $decision_stmt = $db->prepare($decision_sql);
            $decision_stmt->bind_param("iiiis", $escalation_id, $complaint_id, $cost_sharing_id, $campus_registrar_id, $decision_text);
            $decision_stmt->execute();
            $decision_stmt->close();

            // Notify the Campus Registrar
            $notify_registrar_sql = "INSERT INTO notifications (user_id, complaint_id, description) VALUES (?, ?, ?)";
            $notification_desc = "Complaint #$complaint_id has been escalated to you by Cost Sharing: $decision_text";
...
<head>
    ...
    <title>Decide on Complaint #<?php echo $complaint_id; ?> | DMU Registrar Complaint System</title>
