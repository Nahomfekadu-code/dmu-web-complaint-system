...
// Role check: Ensure the user is logged in and is 'hrm'
...
$sql_hrm = "SELECT fname, lname, email, role FROM users WHERE id = ?";
$stmt_hrm = $db->prepare($sql_hrm);
if ($stmt_hrm) {
    $stmt_hrm->bind_param("i", $hrm_id);
    $stmt_hrm->execute();
    $result_hrm = $stmt_hrm->get_result();
    if ($result_hrm->num_rows > 0) {
        $hrm = $result_hrm->fetch_assoc();
    } else {
        $_SESSION['error'] = "Human Resource Management details not found.";
        header("Location: ../logout.php");
        exit;
    }
    $stmt_hrm->close();
} else {
    error_log("Error preparing Human Resource Management query: " . $db->error);
...
$complaint_query = "
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.escalated_to = 'hrm'";
...
$stmt_complaint->bind_param("iii", $complaint_id, $escalation_id, $hrm_id);
...
if ($complaint_result->num_rows === 0) {
    ...
    $debug_info = "Complaint ID: $complaint_id, Escalation ID: $escalation_id, HRM ID: $hrm_id\n";
...
// Fetch the user who escalated the complaint to Human Resource Management (e.g., Cost Sharing)
$escalator = null;
...
if ($send_back_to_escalator_id) {
    ...
    if ($escalator_result->num_rows > 0) {
        ...
        if (in_array($escalator_role, ['cost_sharing', 'handler'])) {
...
// Fetch users for escalation (only Administrative VP)
$roles_to_escalate = ['administrative_vp'];
$escalation_options = [];
...
function sendStereotypedReport($db, $complaint_id, $sender_id, $report_type, $additional_info = '') {
    ...
    if ($sender_result->num_rows === 0) {
        error_log("Sender #$sender Kacper_id not found for report generation.");
    ...
    if (empty($errors)) {
        ...
        try {
            if ($action === 'resolve') {
                ...
                $notification_desc = "Your complaint #$complaint_id has been resolved by {$hrm['fname']} {$hrm['lname']}: $resolution_details";
                ...
                // Notify the escalator (e.g., Cost Sharing or Handler)
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $hrm_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you escalated, has been resolved by {$hrm['fname']} {$hrm['lname']}: $resolution_details";
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $hrm_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $notification_desc = "Complaint #$complaint_id, which you handled, has been resolved by {$hrm['fname']} {$hrm['lname']}: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $hrm_id, 'resolved', $additional_info)) {
                ...
                $stmt_decision->bind_param("iiiis", $complaint_id, $escalation_id, $hrm_id, $send_back_to_id, $resolution_details);
                ...
                $res_detail = "Sent back to $send_back_to_role by Human Resource Management with decision: $resolution_details";
                ...
                $recipient_notification_desc = "A decision requires your attention for Complaint #$complaint_id (sent by Human Resource Management): $resolution_details";
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been reviewed by Human Resource Management and sent back to the $send_back_to_role for further action.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($other_party_id && $other_party_id != $hrm_id && $other_party_id != $send_back_to_id) {
                    $notify_other_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $other_notification_desc = "Complaint #$complaint_id has been sent back to the $send_back_to_role by Human Resource Management: $resolution_details";
                ...
                if (!sendStereotypedReport($db, $complaint_id, $hrm_id, 'decision_sent_back', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }
                ...
                $stmt_escalation->bind_param("isiii", $complaint_id, $escalated_to_role, $escalated_to_id, $hrm_id, $complaint['original_handler_id']);
                ...
                $res_detail = "Escalated to Administrative Vice President by Human Resource Management. Reason: $resolution_details";
                $update_escalation_stmt->bind_param("si", $res_detail, $escalation_id);
                $update_escalation_stmt->execute();
                $update_escalation_stmt->close();
                ...
                // Notify the escalated-to user (Administrative VP)
                $notify_escalated_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $escalated_notification_desc = "Complaint #$complaint_id has been escalated to you (Administrative Vice President) by Human Resource Management for review. Reason: $resolution_details";
                $notify_escalated_stmt = $db->prepare($notify_escalated_sql);
                if (!$notify_escalated_stmt) {
                    throw new Exception("An error occurred while notifying the Administrative Vice President.");
                }
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been escalated to the Administrative Vice President by Human Resource Management.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($can_send_back_to_escalator && $send_back_to_escalator_id != $hrm_id) {
                    $notify_escalator_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $escalator_notification_desc = "Complaint #$complaint_id, which you escalated, has been further escalated to the Administrative Vice President by Human Resource Management. Reason: $resolution_details";
                    $notify_escalator_stmt = $db->prepare($notify_escalator_sql);
                    if (!$notify_escalator_stmt) {
                        throw new Exception("An error occurred while notifying the escalator.");
                    }
                ...
                if ($can_send_back_to_handler && $send_back_to_handler_id != $hrm_id && $send_back_to_handler_id != $send_back_to_escalator_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $handler_notification_desc = "Complaint #$complaint_id, which you handled, has been escalated by Human Resource Management to the Administrative Vice President. Reason: $resolution_details";
                    $notify_handler_stmt = $db->prepare($notify_handler_sql);
                    if (!$notify_handler_stmt) {
                        throw new Exception("An error occurred while notifying the handler.");
                    }
                ...
                // Send stereotyped report to the President
                $additional_info = "Escalated by Human Resource Management to Administrative Vice President. Reason: $resolution_details";
                if (!sendStereotypedReport($db, $complaint_id, $hrm_id, 'escalated', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }

                $_SESSION['success'] = "Complaint #$complaint_id has been escalated to the Administrative Vice President successfully.";
            }
...
if ($notif_stmt) {
    $notif_stmt->bind_param("i", $hrm_id);
    $notif_stmt->execute();
    ...
    <nav class="vertical-nav">
        <div class="nav-header">
            ...
            <div class="user-profile-mini">
                ...
                <div class="user-info">
                    <h4><?php echo htmlspecialchars($hrm['fname'] . ' ' . $hrm['lname']); ?></h4>
                    <p><?php echo htmlspecialchars(ucfirst(str_replace('_', ' ', $hrm['role']))); ?></p>
                </div>
            </div>
        </div>
            ...
            <form method="POST" action="decide_complaint.php?complaint_id=<?php echo $complaint_id; ?>&escalation_id=<?php echo $escalation_id; ?>" novalidate>
                ...
                <div class="form-group">
                    ...
                    <select name="action" id="action" required onchange="toggleAdditionalOptions()">
                        ...
                        <option value="escalate" <?php echo $display_action === 'escalate' ? 'selected' : ''; ?>>Escalate to Administrative Vice President</option>
                    </select>
                </div>
                ...
                <div id="escalation-options">
                    <div class="form-group">
                        <label for="escalation_target_select">Select Administrative Vice President *</label>
                        <select name="escalation_target_select" id="escalation_target_select" onchange="updateEscalationFields(this)">
                            <option value="" disabled selected>-- Select Administrative Vice President --</option>
                            <?php if (!empty($escalation_options['administrative_vp'])): ?>
                                <?php foreach ($escalation_options['administrative_vp'] as $user_option): ?>
                                    <?php
                                        $option_value = 'administrative_vp|' . $user_option['id'];
                                        $is_selected = ($display_escalated_role === 'administrative_vp' && (int)$display_escalated_id === $user_option['id']);
                                    ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <option value="" disabled>No Administrative Vice Presidents found.</option>
                            <?php endif; ?>
                        </select>
            ...
            if (form) {
                form.addEventListener('submit', function(event) {
                    ...
                    if (action === 'resolve') {
                        ...
                        confirmMessage = 'Are you sure you want to escalate this complaint to the Administrative Vice President?';
                    }
