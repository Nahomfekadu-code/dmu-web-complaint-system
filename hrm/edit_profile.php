...
// Fetch Human Resource Management details (including fields for the form)
$sql_user = "SELECT fname, lname, email, username, role FROM users WHERE id = ?";
$stmt_user = $db->prepare($sql_user);
if ($stmt_user) {
    $stmt_user->bind_param("i", $hrm_id);
    $stmt_user->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!isset($_SESSION['error']) && empty($new_email)) {
        ...
        if ($check_email_stmt) {
            $check_email_stmt->bind_param("si", $new_email, $hrm_id);
    ...
    if (!isset($_SESSION['error']) && empty($new_username)) {
        ...
        if ($check_username_stmt) {
            $check_username_stmt->bind_param("si", $new_username, $hrm_id);
        ...
        try {
            ...
            if ($fname_changed || $lname_changed || $email_changed || $username_changed || $password_updated) {
                ...
                if ($password_updated) {
                    ...
                    $update_stmt->bind_param("sssssi", $new_fname, $new_lname, $new_email, $new_username, $hashed_new_password, $hrm_id);
                } else {
                    $update_stmt->bind_param("ssssi", $new_fname, $new_lname, $new_email, $new_username, $hrm_id);
                }
