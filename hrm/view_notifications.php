...
if (isset($_GET['mark_read']) && is_numeric($_GET['mark_read'])) {
    ...
    if ($update_stmt) {
        $update_stmt->bind_param("ii", $notification_id, $hrm_id);
        $update_stmt->execute();
...
if (isset($_GET['mark_all_read']) && $_GET['mark_all_read'] === '1') {
    ...
    if ($update_all_stmt) {
        $update_all_stmt->bind_param("i", $hrm_id);
...
$notif_query = "
    ...
    LEFT JOIN escalations e ON c.id = e.complaint_id AND e.escalated_to = 'hrm' AND e.escalated_to_id = ?
    WHERE n.user_id = ?
    ORDER BY n.created_at DESC";
$notif_stmt = $db->prepare($notif_query);
if ($notif_stmt) {
    $notif_stmt->bind_param("ii", $hrm_id, $hrm_id);
    $notif_stmt->execute();
...
if ($unread_stmt) {
    $unread_stmt->bind_param("i", $hrm_id);
    $unread_stmt->execute();
...
<head>
    ...
    <title>Notifications | Human Resource Management - DMU Complaint System</title>
