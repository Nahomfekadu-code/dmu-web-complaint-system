...
if ($stmt_handler) { // Brace 1 Open
    ...
    exit;
} // Brace 4 Close  <- This should be the brace around line 40

// --- End Fetch handler details ---


// Fetch notifications for this handler using a prepared statement
$sql_notifications = "SELECT n.id, n.complaint_id, n.description, n.created_at, c.title AS complaint_title
                      FROM notifications n
                      LEFT JOIN complaints c ON n.complaint_id = c.id -- Use LEFT JOIN in case complaint is deleted
                      WHERE n.user_id = ?
                      ORDER BY n.created_at DESC";

$stmt_notifications = $db->prepare($sql_notifications);
$notifications_result = null;
$notifications_data = []; // Store data

if ($stmt_notifications) { // Brace 5 Open
    $stmt_notifications->bind_param("i", $handler_id);
    $stmt_notifications->execute();
    $notifications_result = $stmt_notifications->get_result();
     if ($notifications_result) { // Brace 6 Open
        $notifications_data = $notifications_result->fetch_all(MYSQLI_ASSOC); // Fetch all data
    } // Brace 6 Close
    $stmt_notifications->close();
} else { // Brace 5 Else - Brace 7 Open
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .notification-icon a i.active-page { /* Style for current page icon */
            color: var(--primary); /* Make it stand out */
        }
        ...
        @media (max-width: 768px) {
            ...
            /* Responsive table adjustments */
             ...
             table.responsive-table td { border: none; border-bottom: 1px solid #eee; position: relative; padding-left: 45%; text-align: right; display: flex; justify-content: flex-end; align-items: center; min-height: 40px;} /* Ensure min height */
             ...
             table.responsive-table td.notification-description { /* Keep description left aligned */
                 padding-left: 10px; text-align: left; justify-content: flex-start;
                 border-bottom: 1px solid #eee; /* Add border back for clarity */
             }
             table.responsive-table td.notification-description:before { display: none; } /* Hide label for multiline field */

             table.responsive-table td.actions-cell { border-bottom: none; padding-top: 10px; padding-bottom: 10px; justify-content: flex-end;}
             table.responsive-table td.actions-cell:before { display: none; } /* Hide label for actions */
             table.responsive-table tr td:last-child { border-bottom: none; } /* Remove border from last cell in row */
        }
            ...
            <div class="horizontal-menu">
                ...
                <div class="notification-icon" title="View Notifications">
                    <a href="view_notifications.php" style="color: inherit; text-decoration: none;">
                        <i class="fas fa-bell active-page"></i> <!-- Icon indicates current page -->
                    </a>
                </div>
        ...
        <div class="content-container">
            ...
            <div class="card notifications-list-card">
                <div class="card-header">
                     <i class="fas fa-envelope-open-text"></i> Your Recent Notifications
                </div>
                <div class="card-body">
                            ...
                            <table class="responsive-table">
                                ...
                                <tbody>
                                    <?php foreach ($notifications_data as $notification): ?>
                                        <tr>
                                            <td data-label="ID"><?php echo $notification['id']; ?></td>
                                            <td data-label="Complaint">
                                                <?php if ($notification['complaint_id']): ?>
                                                    <a href="view_complaint.php?complaint_id=<?php echo $notification['complaint_id']; ?>" title="View Complaint #<?php echo $notification['complaint_id']; ?>">
                                                        <?php echo $notification['complaint_title'] ? htmlspecialchars($notification['complaint_title']) : ('#' . $notification['complaint_id']); ?>
                                                    </a>
                                                <?php else: ?>
                                                    <span class="text-muted">N/A</span>
                                                <?php endif; ?>
                                            </td>
                                            <td data-label="Notification" class="notification-description"><?php echo nl2br(htmlspecialchars($notification['description'])); ?></td>
                                            <td data-label="Received"><?php echo date('M j, Y, g:i a', strtotime($notification['created_at'])); ?></td>
                                            <td data-label="Actions" class="actions-cell">
                                                <?php if ($notification['complaint_id']): ?>
                                                <a href="view_complaint.php?complaint_id=<?php echo $notification['complaint_id']; ?>" class="btn btn-info btn-small" title="View Related Complaint">
                                                    <i class="fas fa-eye"></i> <span>View</span>
                                                </a>
                                                <?php endif; ?>
                                                <?php // Add 'Mark as Read' or 'Delete' actions if needed ?>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                    ...
                    <?php else: ?>
                        <p class="text-muted" style="text-align: center; padding: 20px 0;">You have no notifications at the moment.</p>
                    <?php endif; ?>
                </div>
            </div>
