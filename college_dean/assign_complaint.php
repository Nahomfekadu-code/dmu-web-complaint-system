...
$dean_query = "SELECT fname, lname FROM users WHERE id = ?";
$stmt_dean = $db->prepare($dean_query);
$stmt_dean->bind_param("i", $dean_id);
$stmt_dean->execute();
$dean = $stmt_dean->get_result()->fetch_assoc();
$stmt_dean->close();

// Fetch Department Heads for assignment
$dept_heads_query = "SELECT id, fname, lname FROM users WHERE role = 'department_head'";
$dept_heads_result = $db->query($dept_heads_query);
$department_heads = [];
if ($dept_heads_result) {
    while ($row = $dept_heads_result->fetch_assoc()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_complaint'])) {
    ...
    $dept_head_id = filter_input(INPUT_POST, 'dept_head_id', FILTER_VALIDATE_INT);
    $assignment_note = filter_input(INPUT_POST, 'assignment_note', FILTER_SANITIZE_STRING);

    if (!$complaint_id || !$dept_head_id || empty($assignment_note)) {
        $_SESSION['error'] = "Please select a Department Head and provide an assignment note.";
    } else {
        // Insert into escalations table (assignment type)
        $insert_query = "
            INSERT INTO escalations (complaint_id, escalated_to, escalated_to_id, action_type, status, created_at)
        ...
        if ($stmt->execute()) {
            ...
            $notification_query = "
                ...
                VALUES (?, ?, ?, 0, NOW())
            ";
            $notification_desc = "Complaint #$complaint_id has been assigned to you by the College Dean: $assignment_note";
            $stmt_notify = $db->prepare($notification_query);
            $stmt_notify->bind_param("iis", $dept_head_id, $complaint_id, $notification_desc);
            $stmt_notify->execute();
            $stmt_notify->close();

            // Update complaint status
            $update_query = "UPDATE complaints SET status = 'assigned' WHERE id = ?";
            $stmt_update = $db->prepare($update_query);
            $stmt_update->bind_param("i", $complaint_id);
            $stmt_update->execute();
            $stmt_update->close();

            $_SESSION['success'] = "Complaint assigned successfully to the Department Head.";
        } else {
            $_SESSION['error'] = "Failed to assign the complaint. Please try again.";
        }
        $stmt->close();
    }
...
// Fetch complaints that are pending and not yet assigned to a Department Head
$complaints_query = "
    SELECT c.*, u.fname as submitter_fname, u.lname as submitter_lname
    FROM complaints c
    LEFT JOIN escalations e ON c.id = e.complaint_id AND e.escalated_to = 'department_head'
...
<head>
    ...
    <title>Assign Complaints | College Dean | DMU Complaint System</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            ...
            <!-- Complaints Table -->
            <?php if ($complaints_result && $complaints_result->num_rows > 0 && !empty($department_heads)): ?>
                <table>
                    <thead>
                        <tr>
                            <th>ID</th>
                    ...
                    <tbody>
                        <?php while ($complaint = $complaints_result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <button class="btn btn-primary btn-small assign-btn" data-complaint-id="<?php echo $complaint['id']; ?>">
            ...
            <?php else: ?>
                <p>No pending complaints available for assignment, or no Department Heads found.</p>
            <?php endif; ?>
        </div>
        ...
        <div id="assignmentModal" class="modal">
            <div class="modal-content">
                ...
                <h4>Assign Complaint to Department Head</h4>
                <form id="assignmentForm" method="POST" action="">
                    <input type="hidden" name="complaint_id" id="assignmentComplaintId">
                    <div class="form-group">
                        <label for="dept_head_id">Select Department Head:</label>
                        <select name="dept_head_id" id="dept_head_id" required>
                            <option value="">-- Select Department Head --</option>
                            <?php foreach ($department_heads as $dept_head): ?>
                                <option value="<?php echo $dept_head['id']; ?>">
                                    <?php echo htmlspecialchars($dept_head['fname'] . ' ' . $dept_head['lname']); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        ...
                        <textarea name="assignment_note" id="assignment_note" required placeholder="Provide a note for this assignment..."></textarea>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const complaintIdInput = document.getElementById('assignmentComplaintId');
