...
// Count total resolved complaints for pagination
$count_query = "
    SELECT COUNT(*) as total
    FROM escalations e
    JOIN complaints c ON e.complaint_id = c.id
    WHERE e.escalated_to = 'campus_registrar'
...
if (!$stmt_count) {
    ...
    $_SESSION['error'] = "An error occurred while fetching resolved complaints.";
    header("Location: dashboard.php");
    exit;
}
...
$total_pages = max(1, ceil($total_complaints / $complaints_per_page));

// Fetch resolved complaints
$complaints_query = "
    SELECT c.id, c.title, c.category, c.status, c.created_at, c.resolution_date, c.resolution_details,
           u.fname, u.lname
    FROM escalations e
    JOIN complaints c ON e.complaint_id = c.id
...
if (!$stmt_complaints) {
    error_log("Prepare failed for complaints query: " . $db->error);
    $_SESSION['error'] = "An error occurred while fetching resolved complaints.";
    header("Location: dashboard.php");
    exit;
}
$stmt_complaints->bind_param("iii", $registrar_id, $complaints_per_page, $offset);
    ...
    <style>
        ...
        * {
        ...
        @media (max-width: 992px) {
            ...
            .complaints-table th, .complaints-table td { padding: 0.75rem; }
        }
        ...
        @media (max-width: 768px) {
            ...
            h2 { font-size: 1.5rem; }
            .complaints-table { font-size: 0.9rem; }
            .complaints-table th, .complaints-table td { padding: 0.5rem; }
        }
                    ...
                    <tbody>
                        <?php foreach ($complaints as $complaint): ?>
                            <tr>
                                ...
                                <td><span class="status-resolved"><?php echo htmlspecialchars(ucfirst($complaint['status'])); ?></span></td>
                                ...
                                <td>
                                    <a href="view_complaint.php?complaint_id=<?php echo $complaint['id']; ?>" class="btn btn-primary">
