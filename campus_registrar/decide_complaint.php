...
$complaint_query = "
    ...
    WHERE c.id = ? AND e.id = ? AND e.escalated_to_id = ? AND e.escalated_to = 'campus_registrar'";
...
if (!$user_stmt) {
    ...
    $_SESSION['error'] = "An error occurred while fetching complainant details.";
    header("Location: dashboard.php");
    exit;
}
...
if (!$user) {
    error_log("Complainant not found for complaint #$complaint_id. User ID: " . ($complaint['user_id'] ?? 'null'));
    $_SESSION['error'] = "Error: The complainant for this complaint could not be found.";
    header("Location: dashboard.php");
    exit;
}
...
$handler = null;
$send_back_to_id = $complaint['original_handler_id'];
if ($send_back_to_id) {
    ...
    $handler_stmt->bind_param("i", $send_back_to_id);
...
if (!$send_back_to_id || !$handler) {
    error_log("Cannot determine original handler for sending back complaint #$complaint_id. Original Handler ID: " . ($complaint['original_handler_id'] ?? 'null'));
    $_SESSION['error'] = "Error: Cannot determine the original handler to send the decision back to.";
    header("Location: dashboard.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($action === 'send_back' && !$send_back_to_id) {
        $errors[] = "Cannot send back: Original handler could not be determined.";
    ...
    if (empty($errors)) {
        ...
        try {
            if ($action === 'resolve') {
                ...
                if ($send_back_to_id && $send_back_to_id != $registrar_id) {
                    ...
                    $notify_handler_stmt->bind_param("iis", $send_back_to_id, $complaint_id, $notification_desc);
                ...
                $res_detail = "Sent back to original handler by Campus Registrar with decision: $resolution_details";
                ...
                // Notify the original handler
                $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $handler_notification_desc = "A decision requires your attention for Complaint #$complaint_id (sent by Campus Registrar): $resolution_details";
                $notify_handler_stmt = $db->prepare($notify_handler_sql);
                if (!$notify_handler_stmt) {
                    throw new Exception("An error occurred while notifying the handler.");
                }
                $notify_handler_stmt->bind_param("iis", $send_back_to_id, $complaint_id, $handler_notification_desc);
                $notify_handler_stmt->execute();
                $notify_handler_stmt->close();

                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been reviewed by the Campus Registrar and sent back to the original handler for further action.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                // Send stereotyped report to the President
                $additional_info = "Sent back to original handler: $resolution_details";
                if (!sendStereotypedReport($db, $complaint_id, $registrar_id, 'decision_sent_back', $additional_info)) {
                    throw new Exception("Failed to send the report to the President.");
                }

                $_SESSION['success'] = "Decision for Complaint #$complaint_id has been sent back to the original handler successfully.";
                ...
                $res_detail = "Escalated to Academic Vice President by Campus Registrar. Reason: $resolution_details";
                $update_escalation_stmt->bind_param("si", $res_detail, $escalation_id);
                $update_escalation_stmt->execute();
                $update_escalation_stmt->close();
                ...
                $escalated_notification_desc = "Complaint #$complaint_id has been escalated to you by the Campus Registrar for review. Reason: $resolution_details";
                $notify_escalated_stmt = $db->prepare($notify_escalated_sql);
                if (!$notify_escalated_stmt) {
                    throw new Exception("An error occurred while notifying the escalated user.");
                }
                ...
                // Notify the complainant
                $notify_user_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                $user_notification_desc = "Your Complaint (#$complaint_id: {$complaint['title']}) has been escalated to the Academic Vice President by the Campus Registrar.";
                $notify_user_stmt = $db->prepare($notify_user_sql);
                if (!$notify_user_stmt) {
                    throw new Exception("An error occurred while notifying the complainant.");
                }
                ...
                if ($send_back_to_id && $send_back_to_id != $registrar_id) {
                    $notify_handler_sql = "INSERT INTO notifications (user_id, complaint_id, description, is_read, created_at) VALUES (?, ?, ?, 0, NOW())";
                    $handler_notification_desc = "Complaint #$complaint_id, which you handled, has been escalated by the Campus Registrar to Academic Vice President. Reason: $resolution_details";
                    $notify_handler_stmt = $db->prepare($notify_handler_sql);
                    if (!$notify_handler_stmt) {
                        throw new Exception("An error occurred while notifying the handler.");
                    }
                ...
                // Send stereotyped report to the President
                $additional_info = "Escalated by Campus Registrar to Academic Vice President. Reason: $resolution_details";
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-container">
            ...
            <?php echo "Rendering form...<br>"; // Debugging statement to confirm form rendering ?>
            <form method="POST" action="decide_complaint.php?complaint_id=<?php echo $complaint_id; ?>&escalation_id=<?php echo $escalation_id; ?>" novalidate>
                ...
                <div class="form-group">
                    ...
                    <select name="action" id="action" required onchange="toggleEscalationForm()">
                        ...
                        <option value="send_back" <?php echo $display_action === 'send_back' ? 'selected' : ''; ?>>Send Back to Original Handler (<?php echo htmlspecialchars($handler['fname'] . ' ' . $handler['lname']); ?>)</option>
    ...
    <script>
        function toggleEscalationForm() {
            ...
            if (!actionSelect || !escalationOptionsDiv || !escalationTargetSelect || !escalationRoleInput || !escalationIdInput) {
                console.error("One or more form elements are missing in toggleEscalationForm.");
                return;
            }
            ...
            if (form) {
                form.addEventListener('submit', function(event) {
                    ...
                    if (action === 'resolve') {
                        ...
                        confirmMessage = 'Are you sure you want to send this complaint back to the original handler?';
