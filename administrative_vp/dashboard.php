...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['escalate_complaint'])) {
    ...
    $stmt_validate = $db->prepare("
        ...
        WHERE e.complaint_id = ?
          AND e.escalated_to = 'administrative_vp'
    ...
    try {
        ...
        $notification_desc_pres = "Complaint #$complaint_id has been escalated to you by Administrative VP {$vp['fname']} {$vp['lname']}. Reason: $decision_text";
        ...
        if ($submitter_result->num_rows > 0) {
            ...
            $notification_desc_user = "Update on Complaint #$complaint_id: Your complaint has been escalated to the President by Administrative VP {$vp['fname']} {$vp['lname']}. Reason: $decision_text";
        ...
        if ($handler_id && $handler_id != $vp_id && $handler_id != $submitter_id) {
            $notification_desc_handler = "Update on Complaint #$complaint_id (originally handled by you): Escalated to President by Administrative VP {$vp['fname']} {$vp['lname']}. Reason: $decision_text";
        ...
        if ($escalated_by_id && $escalated_by_id != $vp_id && $escalated_by_id != $submitter_id && $escalated_by_id != $handler_id) {
            $notification_desc_dean = "Update on Complaint #$complaint_id (which you escalated): Further escalated to President by Administrative VP {$vp['fname']} {$vp['lname']}. Reason: $decision_text";
        ...
        $additional_info_report = "Escalated by Administrative Vice President {$vp['fname']} {$vp['lname']}. Reason: $decision_text";
...
$sql_count_escalated = "
    ...
    WHERE e.escalated_to = 'administrative_vp'
      AND e.escalated_to_id = ?
      AND e.status = 'pending'
      AND e.action_type IN ('escalation', 'assignment')
      ...
      AND e.id = (
          ...
          WHERE e2.complaint_id = c.id
            AND e2.escalated_to = 'administrative_vp'
...
<head>
    ...
    <title>Administrative VP Dashboard | DMU Complaint System</title>
    ...
    <div class="main-content">
                            ...
                            <tbody>
                                <?php foreach ($decided_complaints as $row): ?>
                                    <tr>
                                        ...
                                        <td data-label="Sent To (Role)">
                                            <?php echo htmlspecialchars($row['receiver_name']) . ' (' . htmlspecialchars(ucfirst(str_replace('_', ' ', $row['receiver_role']))) . ')'; ?>
                                        </td>
                                        ...
                                        <td data-label="Actions">
                                            <a href="view_complaint.php?complaint_id=<?php echo $row['complaint_id']; ?>" class="btn btn-info btn-small" title="View Complaint">
                                                <i class="fas fa-eye"></i> View
                                            </a>
                                        </td>
                                    </tr>
        ...
        <div id="escalateModal" class="modal">
            <div class="modal-content">
                ...
                <form id="escalateForm" method="POST" action="dashboard.php">
                    ...
                    <textarea name="decision_text" id="decision_text" placeholder="Provide the reason for escalating this complaint to the President (10-1000 characters)." required></textarea>
                    <button type="submit" name="escalate_complaint" class="btn btn-warning"><i class="fas fa-arrow-up"></i> Escalate</button>
                </form>
            </div>
        </div>
        ...
        <footer>
            <div class="footer-content">
                <div class="group-name">Developed by Group 7</div>
                <div class="social-links">
                    <a href="https://github.com" target="_blank"><i class="fab fa-github"></i></a>
                    <a href="https://linkedin.com" target="_blank"><i class="fab fa-linkedin"></i></a>
                    <a href="mailto:group7@example.com"><i class="fas fa-envelope"></i></a>
                </div>
                <div class="copyright">© 2024 DMU Complaint System. All rights reserved.</div>
            </div>
        </footer>
    </div>
    ...
    <script>
        ...
        escalateButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                const input = modal.querySelector('#escalateComplaintId');
        ...
        // Form validation for escalation
